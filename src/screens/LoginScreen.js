...
import { loginCall } from '../service/loginCall'
...
export default function LoginScreen({ navigation }) {
  ...
  const onLoginPressed = () => {
    ...
    if (emailError || passwordError) {
      ...
      loginCall(email.value, password.value, navigation);
      // ToastAndroid.show("hello from login button", 1000);
      // var response = callApi('login', 'POST', getJson(email.value, password.value));
      // response.then((response) => {
      //   ToastAndroid.show("got responce from api", 10);
      //   ToastAndroid.show(response, 10);
      //   // console.log("yay!! I am from responce...");
